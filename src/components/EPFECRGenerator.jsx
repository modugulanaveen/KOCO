...
export default function EPFECRGenerator({ company = {}, employees = [] }) {
  ...
  const downloadECRFile = () => {
    if (pfData.length === 0) {
      setErrors(['No data to export']);
      return;
    }

    const monthName = new Date(2024, month - 1).toLocaleString('default', { month: 'long' });
    const content = generateECRFileContent(pfData, company);
    const fileName = `ECR_${monthName}_${year}.txt`;
    ...
    setSuccess(`ECR file downloaded: ${fileName}`);
  };
  ...
  const downloadCSVFile = () => {
    ...
    setSuccess(`CSV file downloaded: ${fileName}`);
  };
    ...
    <div className="epf-container">
      ...
      <div style={{ marginBottom: '16px' }}>
        ...
        <p style={{ color: 'var(--text-light)', fontSize: '14px' }}>
          Manage and generate Employee Provident Fund (EPF) Electronic Challan cum Return
        </p>
      </div>
      ...
      <div style={{
        ...
        <div>
          <strong>First Step:</strong> Upload employee data via the CSV Upload page first, then use this page to calculate PF and generate ECR files.
        </div>
      </div>
                ...
                <tbody>
                  {filteredData.map(record => (
                    <tr key={record.id || record.uan}>
                      ...
                      <td>{formatINR(record.adminCharge)}</td>
                      <td>{formatINR(record.edliAdminCharge)}</td>
                      <td>{formatINR((record.eps || 0) + (record.epfEr || 0) + (record.edli || 0) + (record.adminCharge || 0) + (record.edliAdminCharge || 0))}</td>
                      <td>
                        <button
                          type="button"
